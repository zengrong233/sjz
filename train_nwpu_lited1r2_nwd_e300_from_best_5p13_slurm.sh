...
cd "$PROJECT_DIR"
mkdir -p logs runs/nwpu_lited1r2_nwd_e300
...
WEIGHTS="$PROJECT_DIR/best_pt/nwpu_best.pt"
...
if [ ! -d "$PROJECT_DIR/datasets/NWPU-VHR10/train/images" ]; then
  echo "错误: 未找到 NWPU 数据集: $PROJECT_DIR/datasets/NWPU-VHR10"
  exit 1
fi
...
echo "热启动权重: $WEIGHTS"
echo "输出目录: runs/nwpu_lited1r2_nwd_e300/nwpu_lited1r2_nwd_b6_e300_frombest_${JOB_ID}"
...
python "train_yolo111 copy.py" \
  ...
  --batch 6 \
  --workers 8 \
  --project runs/nwpu_lited1r2_nwd_e300 \
  --name "nwpu_lited1r2_nwd_b6_e300_frombest_${JOB_ID}" \
  --trainer_mode full \
  --enable_ssds \
  --ssds_mode soft \
  --small_area_thr 1024 \
