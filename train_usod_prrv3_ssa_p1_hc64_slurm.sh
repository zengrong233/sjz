...
# USOD 训练脚本（P1-HC64 版）
# 默认口径:
#   - 结构: YOLO11-HFAMPAN-AsDDet-NWD-SmallObject-PRR-v3-SSA-P1-HC64.yaml
#   - 模式: full
...
#   TRAINER_MODE=full
#   PROJECT_OUT=runs/usod_p1_hc64
#   RUN_NAME=usod_p1_hc64_${SLURM_JOB_ID}
#   CLEAR_CACHE=1
...
CFG_FILE="${CFG_FILE:-YOLO11-HFAMPAN-AsDDet-NWD-SmallObject-PRR-v3-SSA-P1-HC64.yaml}"
DATA_YAML="${DATA_YAML:-data_USOD_server2.yaml}"
WEIGHTS_PATH="${WEIGHTS_PATH:-yolo11n.pt}"
TRAINER_MODE="${TRAINER_MODE:-full}"
PROJECT_OUT="${PROJECT_OUT:-runs/usod_p1_hc64}"
RUN_NAME="${RUN_NAME:-usod_p1_hc64_${SLURM_JOB_ID}}"
...
if [ ! -f "${PROJECT_DIR}/train_yolo111 copy.py" ]; then
  ...
  echo "PROJECT_DIR=/share/home/u2415363072/4.17/ultralyticsPro--YOLO11 sbatch train_usod_prrv3_ssa_p1_hc64_slurm.sh"
  exit 2
fi
