...
from ultralytics.nn._init_ import *
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, AsDDet)):  # AsDDet is a Detect-compatible head but not a Detect subclass
            s = 256  # 2x min stride
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m is C3k2 and scale in "mlx":  # for M/L/X sizes
                args[3] = True
            if m is C3k2Lite and scale in "mlx":  # SSA-Lite 版与 C3k2 保持同样的 c3k 强制规则
                args[3] = True
        elif m is AIFI:
            args = [ch[f], *args]
...
# -------------------------------------------------
        ...
        #     args = [c1, c2, *args[1:]]
            ...
            c1 = ch[f[0]]
            c2 = args[0]
            # trans_channels = PyramidPoolAgg 的输出通道数（f[1] 指向 PyramidPoolAgg 层）
            global_ch = ch[f[1]]
            args = [c1, c2, global_ch]
            ...
            c2 = ch[f]
            args = [c2, *args]
        elif m in [FrequencyFocusedDownSampling, ConvNormLayer]:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        ...
        #         n = 1
            ...
            c2 = ch[f]
            args = [c2]
        elif m is RefocusSingle:
            # RefocusSingle: 单分支重聚焦，输入输出通道一致
            c2 = ch[f]
            args = [c2, *args]
...
# -------------------------------------------------
            ...
            c2 = sum(ch[x] for x in f)
        elif m in {Detect, AsDDet, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect}:
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, AsDDet, WorldDetect, v10Detect)):
                return "detect"
