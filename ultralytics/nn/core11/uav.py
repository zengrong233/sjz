...
from ultralytics.nn.modules.conv import Conv, DWConv, RepConv, GhostConv, autopad, LightConv, ConvTranspose
from ultralytics.nn.modules.block import RepC3, C3, C2f, Bottleneck

from ultralytics.nn.core11.uav_block import BottleNeck, BasicBlock_

# __all__ = ['DySample_','SPDConv','MFFF','FrequencyFocusedDownSampling','SemanticAlignmenCalibration']


class DySample_(nn.Module):
...
class FFM(nn.Module):
    ...
    def forward(self, x):
        ...
        # cuFFT 在 fp16 下仅支持 2^n 尺寸，频域分支强制使用 fp32 计算后再回转原 dtype
        x1_f = x1.float()
        x2_f = x2.float()
        ...
        return out * self.alpha.to(out.dtype) + x * self.beta.to(x.dtype)
...
class ImprovedFFTKernel(nn.Module):
    ...
    def forward(self, x):
        ...
        x_fft = x_att.to(x_fft.dtype) * x_fft
        ...
        # 使用通道注意力机制
        channel_weights = self.channel_attention(x_att).to(x_sca.dtype)
...
class SemanticAlignmenCalibration(nn.Module):  # 新的模块名称，更具描述性
    def __init__(self, inc, out_c=None):
        ...
        # 默认保持旧行为；仅在 YAML 显式传参时扩展 P3 通道容量。
...
class P3Fusion(nn.Module):
    ...
    输入:
        x: 两路特征列表，例如 [mfff_p3_feat, sac_p3_feat]
           两路空间分辨率必须一致，通道可不同。
    输出:
        单路融合特征，通道数为 out_c。
    """
    ...
    def forward(self, x):
        if not isinstance(x, (list, tuple)):
            x = [x]
        # 空间对齐：SAC 输出在 P2 scale，MFFF(P3) 在 P3 scale，需上采样到最大分辨率
        max_h = max(t.shape[2] for t in x)
        max_w = max(t.shape[3] for t in x)
        x = [F.interpolate(t, size=(max_h, max_w), mode='bilinear', align_corners=True)
             if t.shape[2] != max_h or t.shape[3] != max_w else t for t in x]
