...
# RS-STOD 训练脚本（P1-NoSAC-FullC2f 变体，建议放在 4.20 使用）
# 默认口径:
#   - 结构: YOLO11-HFAMPAN-AsDDet-NWD-SmallObject-PRR-v3-SSA-P1-NoSAC-FullC2f.yaml
#   - 模式: full
#   - 数据: data_RS_STOD_server2.yaml
#   - 初始权重: best_pt/yolo11n.pt（新结构默认冷启动，避免旧 rsstod_best.pt 的层匹配污染）
#
# 用法:
...
#   TRAINER_MODE=full
#   PROJECT_OUT=runs/rsstod_p1_nosac_fullc2f
#   RUN_NAME=rsstod_p1_nosac_fullc2f_${SLURM_JOB_ID}
#   CLEAR_CACHE=1
#   BATCH_SIZE=6
#   IMG_SIZE=640
#   EPOCHS=300
...
WEIGHTS_PATH="${WEIGHTS_PATH:-/share/home/u2415363072/4.20/ultralyticsPro--YOLO11/best_pt/yolo11n.pt}"
TRAINER_MODE="${TRAINER_MODE:-full}"
PROJECT_OUT="${PROJECT_OUT:-runs/rsstod_p1_nosac_fullc2f}"
RUN_NAME="${RUN_NAME:-rsstod_p1_nosac_fullc2f_${SLURM_JOB_ID}}"
...
if [ ! -f "${PROJECT_DIR}/train_yolo111 copy.py" ]; then
  ...
  echo "PROJECT_DIR=/share/home/u2415363072/4.20/ultralyticsPro--YOLO11 sbatch train_rsstod_prrv3_ssa_p1_nosac_fullc2f_slurm.sh"
  exit 2
fi
