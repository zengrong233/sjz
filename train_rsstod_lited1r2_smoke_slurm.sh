...
# 优先使用 5.3 内数据；若 5.3 未复制 RS-STOD，则回退到 4.25 数据路径。
...
DATA_YAML="$PROJECT_DIR/data_RS_STOD_5p3_runtime_${JOB_ID}.yaml"
cat > "$DATA_YAML" <<EOF
path: $RSSTOD_ROOT
train: images/train
val: images/val
...
echo "初始权重: $WEIGHTS"
echo "输出目录: runs/rsstod_lited1r2_smoke/rsstod_lited1r2_b6_e100_${JOB_ID}"
echo "batch/imgsz/amp: 6/640/false"
echo "============================================"

nvidia-smi || true
python -m py_compile ultralytics/nn/core11/GDM.py ultralytics/utils/loss.py ultralytics/utils/NewLoss/ioulossone.py
...
python "train_yolo111 copy.py" \
  ...
  --batch 6 \
  --workers 8 \
  --project runs/rsstod_lited1r2_smoke \
  --name "rsstod_lited1r2_b6_e100_${JOB_ID}" \
  --trainer_mode full \
  --enable_ssds \
  --ssds_mode soft \
  --small_area_thr 1024 \
