...
cd "$PROJECT_DIR"
mkdir -p logs runs/nwpu_lited1r2_nwd_e300_round2
...
echo "热启动权重: $WEIGHTS"
echo "输出目录: runs/nwpu_lited1r2_nwd_e300_round2/nwpu_lited1r2_nwd_b6_e300_round2_${JOB_ID}"
...
python "train_yolo111 copy.py" \
  ...
  --batch 6 \
  --workers 8 \
  --project runs/nwpu_lited1r2_nwd_e300_round2 \
  --name "nwpu_lited1r2_nwd_b6_e300_round2_${JOB_ID}" \
  --trainer_mode full \
  --enable_ssds \
  --ssds_mode soft \
  --small_area_thr 1024 \
