...
test -d "$RSSTOD_ROOT/images/train" || { echo "缺少 RS-STOD train: $RSSTOD_ROOT"; exit 1; }
test -d "$RSSTOD_ROOT/images/val" || { echo "缺少 RS-STOD val: $RSSTOD_ROOT"; exit 1; }

DATA_YAML="$PROJECT_DIR/data_RS_STOD_5p13_${TAG}_${JOB_ID}.yaml"
cat > "$DATA_YAML" <<EOF
path: $RSSTOD_ROOT
train: images/train
val: images/val
...
echo "OUTPUT: runs/rsstod_lited1r2_ablation_e100/rsstod_${TAG}_b6_e100_${JOB_ID}"
...
python "train_yolo111 copy.py" \
  ...
  --batch 6 \
  --workers 8 \
  --project runs/rsstod_lited1r2_ablation_e100 \
  --name "rsstod_${TAG}_b6_e100_${JOB_ID}" \
  --trainer_mode full \
  --enable_ssds \
  --ssds_mode soft \
  --small_area_thr 1024 \
