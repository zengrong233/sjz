...
EXP="${1:?用法: sbatch train_levirship_1cls_clean_ab_5p30_slurm.sh <b0_nwd|b1_aimlaux>}"
...
cd "$PROJECT_DIR"
mkdir -p logs runs/levirship_1cls_clean_ab

JOB_ID="${SLURM_JOB_ID:-manual}"

DATA_YAML="data_LEVIR-Ship-1cls-clean_5p30.yaml"
...
case "$EXP" in
  b0_nwd)
    ...
    RUN_NAME="levirship_b0_nwd_1cls_clean_b6_e300_${JOB_ID}"
    ;;
  b1_aimlaux)
    MODEL_CFG="YOLO11-HFAMPAN-AsDDet-NWD-SmallObject-PRR-v3-SSA-P1-NoSAC-FullC2f-LiteD1R2-NWD-AIMLAux.yaml"
    RUN_NAME="levirship_b1_aimlaux_w0003_1cls_clean_b6_e300_${JOB_ID}"
    ;;
  *)
    echo "错误: EXP 必须是 b0_nwd 或 b1_aimlaux，当前: $EXP"
    exit 1
    ;;
esac
...
if [ "$EXP" = "b0_nwd" ] && grep -qE '^fd_aux:[[:space:]]*true' "$MODEL_CFG"; then
  echo "错误: B0 要求 NWD-only，但 YAML 启用了 fd_aux"
  exit 1
fi

if [ "$EXP" = "b1_aimlaux" ] && ! grep -qE '^fd_aux:[[:space:]]*true' "$MODEL_CFG"; then
  echo "错误: B1 要求 AIMLAux，但 YAML 未启用 fd_aux"
  exit 1
fi

# 强制清掉 clean 数据集 cache，避免复用旧口径。
find datasets/LEVIR-Ship-1cls-clean -name "*.cache" -delete
find datasets/LEVIR-Ship-1cls-clean -name "*.cache.npy" -delete
...
echo "初始权重: $WEIGHTS"
echo "输出目录: runs/levirship_1cls_clean_ab/$RUN_NAME"
...
echo "[CHECK] clean dataset counts"
python - <<'PY'
from pathlib import Path
root = Path("datasets/LEVIR-Ship-1cls-clean")
...
python "train_yolo111 copy.py" \
  ...
  --workers 8 \
  --seed 0 \
  --project runs/levirship_1cls_clean_ab \
  --name "$RUN_NAME" \
  --trainer_mode full \
  --enable_ssds \
  --ssds_mode soft \
