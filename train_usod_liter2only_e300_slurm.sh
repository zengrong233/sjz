...
cd "$PROJECT_DIR"
mkdir -p logs runs/usod_liter2only_e300

JOB_ID="${SLURM_JOB_ID:-manual}"

MODEL_CFG="YOLO11-HFAMPAN-AsDDet-NWD-SmallObject-PRR-v3-SSA-P1-NoSAC-FullC2f-LiteR2Only.yaml"
...
DATA_YAML="$PROJECT_DIR/data_USOD_5p9_liter2only_${JOB_ID}.yaml"
cat > "$DATA_YAML" <<EOF
path: $USOD_ROOT
train: images/train
val: images/val
...
rm -f "$USOD_ROOT"/labels/*.cache 2>/dev/null || true
...
python "train_yolo111 copy.py" \
  ...
  --batch 4 \
  --workers 8 \
  --project runs/usod_liter2only_e300 \
  --name "usod_liter2only_b4_e300_${JOB_ID}" \
  --trainer_mode full \
  --enable_ssds \
  --ssds_mode soft \
  --small_area_thr 1024 \
