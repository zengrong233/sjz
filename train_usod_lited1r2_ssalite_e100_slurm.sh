...
DATA_YAML="$PROJECT_DIR/data_USOD_5p13_ssalite_e100_${JOB_ID}.yaml"
cat > "$DATA_YAML" <<EOF
path: $USOD_ROOT
train: images/train
val: images/val
...
echo "OUTPUT=runs/usod_lited1r2_ssalite_e100/usod_ssalite_b4_e100_${JOB_ID}"
...
python "train_yolo111 copy.py" \
  ...
  --batch 4 \
  --workers 8 \
  --project runs/usod_lited1r2_ssalite_e100 \
  --name "usod_ssalite_b4_e100_${JOB_ID}" \
  --trainer_mode full \
  --enable_ssds \
  --ssds_mode soft \
  --small_area_thr 1024 \
