...
SEED="${1:?用法: sbatch train_nwpu_nwd_aimlaux_seed_stability_5p13_slurm.sh <seed>}"
...
echo "初始权重: $WEIGHTS"
echo "输出目录: runs/nwpu_lited1r2_nwd_aimlaux_seed_stability/nwpu_nwd_aimlaux_w0003_seed${SEED}_b6_e300_${JOB_ID}"
echo "============================================"
...
python "train_yolo111 copy.py" \
  ...
  --workers 8 \
  --seed "$SEED" \
  --project runs/nwpu_lited1r2_nwd_aimlaux_seed_stability \
  --name "nwpu_nwd_aimlaux_w0003_seed${SEED}_b6_e300_${JOB_ID}" \
  --trainer_mode full \
  --enable_ssds \
  --ssds_mode soft \
  --small_area_thr 1024 \
