#!/bin/bash
#SBATCH --job-name=rsstod_ab_p3fb
...
DATA_YAML="$PROJECT_DIR/data_RS_STOD_4p25_runtime_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.yaml"

PROJECT_OUT="runs/rsstod_p1_nosac_fullc2f_warmup_p3fb_ab"
...
case "${SLURM_ARRAY_TASK_ID}" in
  0)
    EXP_ID="A_warmup0_no_p3fb"
    RUN_NAME="rsstod_nosac_fullc2f_A_warmup0_no_p3fb_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
    EXTRA_ARGS=(
      --name "$RUN_NAME"
    )
    ;;
  1)
    EXP_ID="B_warmup0_p3fb_score010"
    RUN_NAME="rsstod_nosac_fullc2f_B_warmup0_p3fb010_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
    EXTRA_ARGS=(
      --name "$RUN_NAME"
      --ssds_p3_fallback
      --ssds_p3_fallback_topk 1
      --ssds_p3_fallback_score 0.1
      --ssds_p3_fallback_min_area 128
      --ssds_p3_fallback_max_area 1024
    )
    ;;
  *)
    echo "错误: 未知 SLURM_ARRAY_TASK_ID=${SLURM_ARRAY_TASK_ID}"
    exit 1
    ;;
esac
...
python "train_yolo111 copy.py" "${COMMON_ARGS[@]}" "${EXTRA_ARGS[@]}"
