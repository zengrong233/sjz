...
cd "$PROJECT_DIR"
mkdir -p logs runs/usod_lited1r2_ciou_e300

JOB_ID="${SLURM_JOB_ID:-manual}"

MODEL_CFG="YOLO11-HFAMPAN-AsDDet-NWD-SmallObject-PRR-v3-SSA-P1-NoSAC-FullC2f-LiteD1R2-CIoU.yaml"
WEIGHTS="$PROJECT_DIR/best_pt/yolo11n.pt"

USOD_ROOT="${USOD_ROOT:-$PROJECT_DIR/datasets/USOD}"
if [ ! -d "$USOD_ROOT/images/train" ]; then
  USOD_ROOT="/share/home/u2415363072/5.12/ultralyticsPro--YOLO11/datasets/USOD"
fi
if [ ! -d "$USOD_ROOT/images/train" ]; then
  USOD_ROOT="/share/home/u2415363072/5.3/ultralyticsPro--YOLO11/datasets/USOD"
fi
if [ ! -d "$USOD_ROOT/images/train" ]; then
...
DATA_YAML="$PROJECT_DIR/data_USOD_5p13_ciou_e300_${JOB_ID}.yaml"
cat > "$DATA_YAML" <<EOF
path: $USOD_ROOT
train: images/train
val: images/val
...
echo "初始权重: $WEIGHTS"
echo "输出目录: runs/usod_lited1r2_ciou_e300/usod_ciou_b4_e300_${JOB_ID}"
...
python "train_yolo111 copy.py" \
  ...
  --batch 4 \
  --workers 8 \
  --project runs/usod_lited1r2_ciou_e300 \
  --name "usod_ciou_b4_e300_${JOB_ID}" \
  --trainer_mode full \
  --enable_ssds \
  --ssds_mode soft \
  --small_area_thr 1024 \
